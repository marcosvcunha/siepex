...
class SelececionarPartRustica extends StatefulWidget {
  final List<ParticipanteRustica> participantes;
  final int position; // Posição sendo selecionada

  SelececionarPartRustica({this.participantes, this.position});
  @override
  _SelececionarPartRusticaState createState() =>
      _SelececionarPartRusticaState();
}
          ...
          itemBuilder: (context, index) {
            if (index == 0)
              return Padding(
                  ...
                  child: Text(
                    'Selecione na lista abaixo o participante que finalizou na posição ${widget.position}:',
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.w500),
                  ));
            return ListTile(
              title: Text(widget.participantes[index - 1].nome),
              subtitle: Text(widget.participantes[index - 1].celularFormated),
              onTap: () {
                print('Retornando');
                Navigator.pop(context, widget.participantes[index - 1].cpf);
              },
            );
          },
        ),
